...
class TransformControls extends Controls<{}> {
   ...
   */
  constructor(camera: Camera, domElement: HTMLElement | null = null) {
  ...
  pointerHover(pointer) {
    ...
    if (this.mode === 'all') {
      // Check all pickers in order: scale, rotate, translate
      // This order ensures smaller handles (scale cubes) are prioritized
  ...
  pointerDown(pointer) {
    ...
    if (this.axis !== null) {
      ...
      // Determine which mode is active when in 'all' mode
      if (this.mode === 'all') {
        // Check which picker was intersected to determine the active mode
      ...
      if (planeIntersect) {
        ...
        // Use bounding box center for worldPositionStart
        const boundingBoxCenter = calculateBoundingBoxCenter(this.object)
        this.worldPositionStart.copy(boundingBoxCenter)

        // Still need to get rotation and scale from the object's matrix
        this.object.matrixWorld.decompose(
          this._tempPosition, // Use temporary variable for position
  ...
  _updateDistanceOverlay() {
    ...
    if (
      ...
      // Position the label at the midpoint between start and current position
...
// Utility function to calculate the bounding box center of an object
export function calculateBoundingBoxCenter(object) {
  const box = new Box3()

  // Compute bounding box of the object and all its children
  ...
  // If no geometry found, fall back to object's world position
  if (box.isEmpty()) {
...
class TransformControlsRoot extends Object3D {
  ...
  updateMatrixWorld(force) {
    ...
    if (controls.object !== undefined) {
      ...
      // Calculate the bounding box center instead of using object's world position
      ...
      controls.object.matrixWorld.decompose(
        controls._tempPosition, // Use temporary variable for position
...
class TransformControlsGizmo extends Object3D {
  constructor() {
    ...
    matRed.color.setHex(0xed4358)

    const matGreen = gizmoMaterial.clone()
    matGreen.color.setHex(0x82cc19)

    const matBlue = gizmoMaterial.clone()
    matBlue.color.setHex(0x3185eb)

    const matRedTransparent = gizmoMaterial.clone()
    matRedTransparent.color.setHex(0xed4358)
    matRedTransparent.opacity = 0.5

    const matGreenTransparent = gizmoMaterial.clone()
    matGreenTransparent.color.setHex(0x82cc19)
    matGreenTransparent.opacity = 0.5

    const matBlueTransparent = gizmoMaterial.clone()
    matBlueTransparent.color.setHex(0x3185eb)
  ...
  updateMatrixWorld(force) {
    ...
    // Show gizmos based on mode
    if (this.mode === 'all') {
      // Show all gizmos simultaneously
    ...
    let handles = []
    if (this.mode === 'all') {
      // Include all pickers and gizmos
